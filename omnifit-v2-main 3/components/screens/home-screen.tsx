...
import { useStore, ROUTINE } from '@/lib/store'

const DAY_LABELS = ['M','T','W','T','F','S','S']
const DAY_SCHEDULE: Record<string,string> = { push:'Mon & Thu', pull:'Tue & Fri', legs:'Wed & Sat' }

export function HomeScreen() {
  const { userName, level, xp, xpToNext, streak, totalWorkouts,
    playerHp, playerMaxHp, bossName, bossHp, bossMaxHp,
    activeDay, sessionLogs, today, setScreen, setDay } = useStore()
      ...
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="text-xl font-bold tracking-tight">{userName || 'Player'}</h1>
          </div>
        </div>
