...
export function LoginScreen() {
  ...
  // got redirected back from the OAuth callback), and if so, look
  // up whether this account already has a username saved. Returning
  // users skip the username prompt entirely and go straight in.
  useEffect(() => {
    async function checkSession() {
      ...
      if (currentUser) {
        ...
        if (profile?.username) {
          // Returning user with a saved username — skip the prompt.
  ...
  async function saveUsernameAndEnter(name: string | null) {
    if (!user) return
    setSaving(true)
    setError('')
      ...
      <div className="flex flex-col gap-5 px-6 py-6 flex-1">
        {!user ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <button
              disabled={saving}
              onClick={() => saveUsernameAndEnter(username.trim() || null)}
              className="w-full bg-primary text-primary-foreground font-bold text-sm rounded-xl py-4 neon-glow active:scale-95 transition-transform uppercase tracking-wide disabled:opacity-60"
            >
              {saving ? 'Saving…' : "LET'S GO →"}
            </button>
            <button
              disabled={saving}
              onClick={() => saveUsernameAndEnter(null)}
              className="w-full text-center text-xs text-muted-foreground underline underline-offset-2 disabled:opacity-60"
            >
              Skip for now
            </button>
          </div>
